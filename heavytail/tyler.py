...
This code implements Tyler's estimator, which is a robust estimator of the covariance matrix suitable for
heavy-tailed distributions.
It is particularly useful in scenarios where traditional estimators may fail due to the presence of outliers or
...
def tyler_covariance(
    data: np.ndarray, epsilon: float = 1e-6, tol: float = 1e-6, max_iter: int = 100
) -> np.ndarray:
    ...
    for _ in range(max_iter):
        ...
        weights = 1 / np.maximum(
            data_inv_cov_data, epsilon
        )  # Shape(T, ). Avoid division by zero
        # Update covariance matrix
        cov = (n_samples / data_cen.shape[0]) * (
            data_cen.T @ np.diag(weights) @ data_cen
        )  # Shape (N, N)
