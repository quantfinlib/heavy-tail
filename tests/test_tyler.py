...
def generate_heavytail_data(n_samples, n_features, nu) -> np.ndarray:
    ...
    return multivariate_t.rvs(df=nu, loc=mu, shape=sigma_scatter, size=n_samples, random_state=rng)
...
def heavytail_data():
    ...
    return generate_heavytail_data(n_samples, n_features, nu)
...
def test_tyler_vs_scm_error(n_samples, n_features, nu):
    ...
    nu : float
        Degrees of freedom for the multivariate t-distribution.
    """
    x = generate_heavytail_data(n_samples, n_features, nu)

    sigma_scm = np.cov(x, rowvar=False)
    sigma_tyler = tyler_covariance(x)

    scm_error = np.linalg.norm(sigma_scm - np.eye(n_features), ord="fro")
    tyler_error = np.linalg.norm(sigma_tyler - np.eye(n_features), ord="fro")
